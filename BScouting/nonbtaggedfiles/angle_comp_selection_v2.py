...
hist_array_angle = MakeHist(3,colors,names,"Angle between Parton and Jet" ,"Angle (rad)","Normalized Instances",1,0,25)

bins = 25
h1 = ROOT.TH2F("Pt Difference Parton vs Gen", "Pt Difference Parton vs Gen", bins, 0, 1000, bins, 0, 1000)
h2 = ROOT.TH2F("Pt Difference Parton vs Scouting", "Pt Difference Parton vs Scouting", bins, 0, 1000, bins, 0, 1000)
h3 = ROOT.TH2F("Pt Difference Parton vs AK8", "Pt Difference Parton vs AK8", bins, 0, 1000, bins, 0, 1000)
h4 = ROOT.TH2F("Pt Difference Gen vs Scouting", "Pt Difference Gen vs Scouting", bins, 0, 1000, bins, 0, 1000)
...
for ientry in range(tree.GetEntries()):
    ...
    test_array = np.frombuffer(test, dtype = np.float32)

    print(tree.GenPart_mass[0])
    ...
    for gp in range(tree.nGenPart):
        ...
        if tree.GenPart_statusFlags[gp] == 4481 and math.fabs(tree.GenPart_pdgId[gp]) == 5 and math.fabs(tree.GenPart_pdgId[tree.GenPart_genPartIdxMother[gp]]) == 1000024:
            MyParticles.append(vec_creator(tree.GenPart_pt[gp], tree.GenPart_eta[gp], tree.GenPart_phi[gp], tree.GenPart_mass[gp]))
    
    MyJets = []
    for j in range(tree.nScoutingJet):
        MyJets.append(vec_creator(tree.ScoutingJet_pt[j],tree.ScoutingJet_eta[j],tree.ScoutingJet_phi[j],tree.ScoutingJet_mass[j]))
    
    MyGen = []
    for l in range(tree.nGenJet):
        MyGen.append(vec_creator(tree.GenJet_pt[l],tree.GenJet_eta[l],tree.GenJet_phi[l],tree.GenJet_mass[l]))

    MyFatJets = []
    for l in range(tree.nGenJetAK8):
        MyFatJets.append(vec_creator(tree.GenJetAK8_pt[l],tree.GenJetAK8_eta[l],tree.GenJetAK8_phi[l],tree.GenJetAK8_mass[l]))    
